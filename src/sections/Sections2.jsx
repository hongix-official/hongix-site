...
// Product-design showcase (logbill / reamly), generated by build-work.mjs.
const PRODUCT_URL = '/work';

/* One portfolio "track" — an outline row-link with an icon, title, one-line
   descriptor, and a trailing arrow (up-right for external, right for on-site). */
function PortfolioTrack({ href, external, icon, tone, title, sub }) {
  return (
    <a
      className="hx-track"
      href={href}
      {...(external ? { target: '_blank', rel: 'noopener' } : {})}
      style={{ display: 'flex', alignItems: 'center', gap: 14, textDecoration: 'none', color: 'inherit', background: 'var(--paper)', border: 'var(--bw) solid var(--line)', borderRadius: 'var(--radius-lg)', boxShadow: 'var(--shadow-xs)', padding: '13px 15px' }}
    >
      <span style={{ width: 42, height: 42, flex: 'none', display: 'inline-flex', alignItems: 'center', justifyContent: 'center', background: tone, border: '1px solid var(--line)', borderRadius: 'var(--radius-md)', fontSize: 20 }}>
        <i className={`ph-bold ${icon}`} />
      </span>
      <span style={{ flex: 1, minWidth: 0 }}>
        <span style={{ display: 'block', fontFamily: 'var(--font-display)', fontWeight: 800, fontSize: 16, letterSpacing: '-.01em', lineHeight: 1.2 }}>{title}</span>
        <span style={{ display: 'block', fontSize: 12.5, color: 'var(--ink-soft)', marginTop: 2 }}>{sub}</span>
      </span>
      <i className={`ph-bold ${external ? 'ph-arrow-up-right' : 'ph-arrow-right'} hx-track__arrow`} style={{ fontSize: 16, color: 'var(--ink-faint)', flex: 'none' }} />
    </a>
  );
}
      ...
      <div className="hx-container">
        ...
        <div className="hx-portfolio" style={{ display: 'grid', gridTemplateColumns: '1.5fr 1fr', border: 'var(--bw) solid var(--line)', borderRadius: 'var(--radius-xl)', boxShadow: 'var(--shadow-lg)', overflow: 'hidden', minHeight: 296, background: 'var(--paper)' }}>
          ...
          <div style={{ padding: 'var(--space-6)', display: 'flex', flexDirection: 'column', justifyContent: 'center', gap: 14 }}>
            ...
            <p style={{ fontSize: 'var(--text-md)', color: 'var(--ink-soft)', margin: 0, maxWidth: '34ch' }}>Two tracks. Let the work speak for itself.</p>
            <div style={{ display: 'flex', flexDirection: 'column', gap: 10, marginTop: 6 }}>
              <PortfolioTrack href={PORTFOLIO_URL} external icon="ph-browser" tone="var(--yellow-300)" title="Website design" sub="Brands, sites & landing pages" />
              <PortfolioTrack href={PRODUCT_URL} icon="ph-app-window" tone="var(--coral-300)" title="Product design" sub="Live apps & product UX" />
            </div>
          </div>
        </div>
      </div>
